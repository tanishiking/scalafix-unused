...
trait BadAPI extends InterFace {
  def f(a: Int, b: String, c: Double)(implicit s: String): Int = { // assert: Unused
    println(b + c)
    a
  }
  @deprecated("no warn in deprecated API", since = "yesterday")
  def g(a: Int, b: String, c: Double)(implicit s: String): Int = { // assert: Unused
    println(b + c)
    a
  }
  override def call(a: Int, b: String, c: Double)(implicit s: String): Int = { // assert: Unused
    println(b + c)
    a
  }
